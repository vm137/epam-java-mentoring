...
from students
         join exam_results er on students.code = er.student_code and code = $1
group by code;
$$ language sql;
...
create or replace function find_red_zone_students()
    returns table
            (
...
select student_code
from students
         join exam_results er on students.code = er.student_code
where mark <= 3
group by student_code
having count(student_code) >= 2;
$$ language sql;
